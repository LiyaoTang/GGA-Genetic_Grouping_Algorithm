...
public class TimeWindowGP2 extends GPRuleBase implements ITimeWindowRule{
	 @Override
     	...
     	CN = cell.getProcessingMachine().size();
	        ...
	        double result =  Add (Add (Div (Add (NMRT, NDD) ,Div (NW ,NDD)),
	        	     Div (Div (Add (Div (Add (Div (Add( NMRT ,NDD),
	        	             Div (NW, NDD)), Mul( NDD,Mul ( SW, NDD))), Div
	        	             (Mul (Mul (Add (Div( NW, NDD), NW), Mul (NW ,NDD)),
	        	                 Div (Div (NRET, Mul( NMRT, NDD)) ,Sub (Div
	        	                     (Sub (Mul (NMRT, NDD), Div( CSD, SW)) ,NW), NRT))),
	        	             Add (Sub( BN, CSW) ,Sub (Div (Mul( NMRT, NDD),
	        	                 Add (NMRT, NDD)), Div (CSD ,SW))))) ,Div (Mul
