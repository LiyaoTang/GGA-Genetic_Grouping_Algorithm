...
public  class SimpleScheduler extends AbstractScheduler{    
    ...
    public void assignOperationToMachine(Job job){
        ...
        if (nextScheduleOperation != null) {
            ...
            selectedMachine.addOperationToBufferIn(nextScheduleOperation);
    ...
    public void ruleBufferOut(Cell cell){
    	...
    	IBufferOutRule rule = (IBufferOutRule)bChromosome.getEntityRule(3,cell.GetID()); // get transporting rule for this cell

		// re-sort the buffer according to the rule
	...
	/** 为operation 选 machine */
    public void jobRule(IJobRule rule,Operation operation){
        double max = -Double.MAX_VALUE;
        Machine selectedMachine = null;

		// 遍历 machineSet 依据rule选择最优score对应的machine
    ...
    public void unload(Machine machine){
    	...
    	if(operation != null && machine.getState() == 1){
    		Job job = operation.getJob();
    		job.scheduleOperation(); // operation is done: nextScheduleNo++;

    		if(job.getNextScheduleOperation() != null){
    		    jobRule((IJobRule)jChromosome.getEntityRule(2,job.getId()), job.getNextScheduleOperation()); // select machine for next operation
    			if(machine.getNumInCell() == job.getNextScheduleOperation().getSelectedMachine().getNumInCell()){ // doesn't need transporting
    				assignOperationToMachine(job);
    			}
    			else{ // need tranporting
    				Cell locatedCell = null;
    				for(Cell cell:cellSet){
    					if(cell.GetID() == machine.getNumInCell())
    						locatedCell = cell; // current cell
    				}
    				locatedCell.AddOperationToBufferOut(job.getNextScheduleOperation()); // operation (job) waits for vehicle
    				
    				job.getNextScheduleOperation().setTransferCarId_to();
				...
				job.setJobIdle(true);
				job.setFinishTime(Timer.currentTime());
    		}
    	}
    }
    ...
    public void load(Machine machine){
    	if(machine.getBufferIn().size() != 0 && machine.getState() == 0){
    		// machine selects operation according to its rule
    		machineRule((IMachineRule)mChromosome.getEntityRule(1,machine.getId()) /** rule for machine */, machine);
    ...
    public void scheduleVehicle(Cell cell){
    	if(cell.getBufferOut().size() != 0){
			/** 大于运输能力: 从缓冲区中选取小车体积个 */
    		if(cell.getBufferOut().size() > cell.GetVehicle().getCapacity()){
    			...
    			for(i = 1;i < cell.GetVehicle().getCapacity();i++){
    				...
    				if(j == i){
	        			...
	        			for(Cell c : cellSet){
	        				if(c.GetID() == desCell.get(desTime.indexOf(max))) lastCell = c;
	        			}
				...
				// calculate and set vehicle back time
    			int max = 0;
    			for(int k : desTime){
    				if(k > max) max = k;
    			}
				...
				// remove operations on transporting'; from buffer
			...
			/** 小于等于运输能力: 清空缓冲区 */
    			...
    			if((cell.GetVehicle().getTimeWindowIdle() == true)&&(cell.GetVehicle().getTimeWindow() == Timer.currentTime())){	
    				...
    				for(int i = 0;i < cell.getBufferOut().size();i++){ // add all operation into vehicle
    					Operation operation = cell.getBufferOut().get(i);	
        				cell.GetVehicle().addOperation(operation);
    				}
    				List<Integer> desCell = new ArrayList<Integer>();
					...
					desTime.add(cell.GetTransferTime(cell.getBufferOut().get(0).getSelectedMachine().getNumInCell()) /** transfer time from current cell to the 1st des cell */ + Timer.currentTime());
					Timer.addTrigger(cell.GetTransferTime(cell.getBufferOut().get(0).getSelectedMachine().getNumInCell())+Timer.currentTime());
    				...
    				for(i = 1;i < cell.getBufferOut().size();i++){
    					...
    					if(j == i){
    	        			...
    	        			Cell lastCell = new Cell();
    	        			for(Cell c : cellSet){
    	        				if(c.GetID() == desCell.get(desTime.indexOf(max))) {lastCell = c; break;}
    	        			}
    	        			desTime.add(max+lastCell.GetTransferTime(desCell.get(i)));
    	        			Timer.addTrigger(max+lastCell.GetTransferTime(desCell.get(i)));
    					}
    ...
    public void scanVehicle(){
    	for(Cell cell : cellSet){     
    		...
    		if(cellVehicle.getIdle() == true /** vehicle available */){
    			ruleBufferOut(cell); // re-sort the buffer according to the rule
    			scheduleVehicle(cell); // set descell, destime according to the sequence in buffer
    		}

			// some job arrive their dest
    		if((cellVehicle.getIdle() == false /** vehicle busy */) && (cellVehicle.IsVehicleArrival(Timer.currentTime()) != 0)){
    ...
    public void scheduleStart(){
    	...
    	for (Job job : jobSet) {
			...
			// add job into machine's buffer (job's waiting for machine)
			this.assignOperationToMachine(job);
		}
    }
	...
	public void schedule() {
		...
		//分配job第一道工序
		scheduleStart(); // job select machine for it's each operation; job is added into machine's buffer (depending on the next operation)
		scanMachine(); // machine select operation (job) from its buffer; timer is set
		scanVehicle(); // available vehicle load jobs or busy vehicle arrive at dest cell or
