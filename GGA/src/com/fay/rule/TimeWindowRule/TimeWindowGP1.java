...
public class TimeWindowGP1 extends GPRuleBase implements ITimeWindowRule{
	 @Override
	    ...
	    public double calPrio(Cell cell) {
	        ...
	        double result = 
	        		Add (Add (Div (Add (NMRT, NDD), Div (NW, NDD)),
	   	        	     Div (Div (Add (Div (Add (Div (Add (NMRT ,NDD),
	   	        	             Div( NW ,NDD)), Div (Mul (NMRT, NDD),Div  (NW,
	   	        	             NDD))),Div ( NW, NDD)), Div (Mul (Mul (Add
	   	        	             (Div( NW, NDD),NW) ,Mul( NW ,NDD)), Mul( NW ,NDD)),
	   	        	             Mul (Mul( NW ,NDD), Mul (NW ,NDD)))), Add (Sub
	   	        	             (BN, CSW), Add (SRT, SD))), Add (Sub (BN ,CSW),
	   	        	             Add( SRT, Add (Sub (BN, CSW), Div (Add (SRT,
	   	        	                 Add (Sub( BN ,CSW) ,Div (Mul( NMRT, NDD) ,Sub
	   	        	                     (Div (Mul( NMRT, NDD), NW), NRT)))) ,Sub (Div
	   	        	                 (Add( NMRT ,Add (Add (Div (Add( NMRT ,NDD) ,Div
	   	        	                     (NW, NDD)), CSW), Mul (Add (Mul (NRET,Mul ( NMRT,
	   	        	                     NDD)) ,NW), Sub( NRT, NRT)))), NW), NRT))))))),
