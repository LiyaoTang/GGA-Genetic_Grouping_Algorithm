...
//import ec.app.myApp.EvolveLocalSearchRule;
...
//        public static EvolveLocalSearchRule localProblem;
...
//        PrintFuncResult();
//    	FormPriorSequences(caseNo,machineNo);
//        log.info("————————————————————————————————GA当前代试验结束————————————————————————————————");
    }
     ...
     */
    private void WriteToFile(int caseNo,int k, StringBuilder[] machines, MyLinkedList[] cells, StringBuilder[][] cellToParts)throws CloneNotSupportedException, IOException {
    	BufferedWriter bw;

    		//if the dir is exists
    		File file = new File(Constants.GPSET_DIR);
    		if (!file.exists()) {
    			   file.mkdir();
    		}
    		//write the file
...
//					));
    			...
    			File file2 = new File(Constants.GPSET_DIR+"/"+(caseNo+1));
        		if (!file2.exists()) {
        			   file2.mkdir();
        		}
        			bw = new BufferedWriter(new FileWriter(Constants.GPSET_DIR+"/"+(caseNo+1)+"/"+(k+1)));
...
//    	    			System.out.println("cell"+i+":"+cells[i].toString()+"\n");
     ...
     */
    private void FormPriorSequences(int caseNo,int MachineNo) throws CloneNotSupportedException, IOException {
...
//    	}
    	...
    	WriteToFile(caseNo,MachineNo,machines,Cells,CellToParts);
	}
...
//        Timer.printTrigger();
	...
	public void scheduleWithCaseNo(int caseNo,int machineNo) throws CloneNotSupportedException, IOException {
	    reset();        // 重置调度状态
        Timer.resetTimer();
        initialTrigger();
